<div
	class="bg-surface-100-800-token w-screen flex flex-col md:flex-row justify-between py-[10vh] px-[10vw]"
>
	<div class="flex flex-col h-32">
		<img class="w-8 h-8" src="/SBJCTV-Logo.png" alt="SBJCTV-Logo.png" />
		<p>Subjective</p>
		<p>All Rights Reserved</p>
	</div>
	<nav aria-label="socials" class="flex flex-col h-32">
		<p class="font-bold">Socials</p>
		<a href="https://www.instagram.com/subjective/">Instagram</a>
		<a href="https://twitter.com/subjective">Twitter</a>
		<a href="https://www.facebook.com/subjective">Facebook</a>
	</nav>
	<nav aria-label="prodcution" class="flex flex-col h-32">
		<p class="font-bold">Production</p>
		<a href="/branding">Branding</a>
		<a href="/clothing">Clothing</a>
	</nav>
	<nav aria-label="legal" class="flex flex-col h-32">
		<p class="font-bold">Legal</p>
		<a href="/legal#terms-of-use">Terms Of Use</a>
		<a href="/legal#privacy-policy">Privacy Policy</a>
		<a href="/legal#copyright">Copyright</a>
	</nav>
	<div class="flex flex-col h-32">
		<p class="font-bold">Contact Us</p>
		<p>subjectiveteam@gmail.com</p>
	</div>
</div>
